...
async function getDeepSeekPicks({ riskPreference, supply, stake, portfolio }) {
  ...
  const systemPrompt = [
    ...
    "candidates are 'Cirrus Finance' (a second, independently-deployed lending market this same project also runs,",
    "sharing Stratus's own reserve tokens) or 'Bonzo Finance' (a genuinely different, third-party lending protocol",
    "on Hedera testnet, not affiliated with this project) — real cross-protocol comparisons, treat them as such",
    "in your reasoning when relevant (e.g. note that Bonzo requires the user's own separate testnet tokens, while",
    "Cirrus Finance uses the same tokens the user already holds on Stratus).",
