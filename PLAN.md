...
**Gate 2: partially passed.** Deposit/withdraw work exactly against the raw pool, with real tx hashes (`deployments/hederaTestnet.json` → `meta.gate2DepositWithdraw`). Borrow/repay — and thus the full gate — blocked on Pyth infrastructure, not on anything in this codebase: got a Hermes API key, fixed a real Hedera tinybar-precision bug in `updatePriceFeeds` along the way (redeployed oracle, re-wired into the pool), but the on-chain Pyth contract itself now rejects fresh update data with `InvalidWormholeVaa()` — confirmed by calling it directly, bypassing our code entirely. Likely a post-upgrade sync gap on Hedera testnet's Pyth deployment. Paused here by agreement (2026-09-07); proceeding to Phase 3, which doesn't need a live oracle read. See `docs/phase-2-findings.md`.
